(function() {
  ...
  stopButton.style.all = 'revert';
  container.appendChild(stopButton);
  container.appendChild(document.createElement('br'));
  container.appendChild(document.createElement('br'));
  
  // "자동 녹음 중지" 체크박스 및 레이블 생성 (기본 브라우저 스타일 적용)
  const autoStopCheckbox = document.createElement('input');
  autoStopCheckbox.checked = true;
  autoStopCheckbox.type = 'checkbox';
  autoStopCheckbox.id = 'autoStopCheckbox';
  autoStopCheckbox.style.all = 'revert';
  container.appendChild(autoStopCheckbox);
  
  const autoStopLabel = document.createElement('label');
  autoStopLabel.htmlFor = 'autoStopCheckbox';
  autoStopLabel.textContent = '자동 녹음 중지';
  autoStopLabel.style.marginLeft = '5px';
  container.appendChild(autoStopLabel);
  ...
  // --- (3) AudioContext, GainNode, MediaStreamDestination 세팅 ---
  ...
  audioElement.addEventListener('ended', () => {
    if (autoStopCheckbox.checked) {
      ...
      document.evaluate('//button[text()="종료"]', document).iterateNext().click();
    }
  });
  ...
  navigator.mediaDevices.getUserMedia = function(constraints) {
    ...
    return constraints.audio === true ? originalGetUserMedia(constraints) : Promise.resolve(destination.stream);
  };
})();
